...
{
    public partial class PaymentVoucherListViewModel : ObservableObject
    {
        private readonly PaymentVoucherService _service;

        public ObservableCollection<PaymentVoucherLookupItem> Items { get; } = new();

        [ObservableProperty] 
        [NotifyCanExecuteChangedFor(nameof(EditDialogCommand))]
        [NotifyCanExecuteChangedFor(nameof(DeleteCommand))]
        private PaymentVoucherLookupItem? selectedItem;
        ...
        public PaymentVoucherListViewModel(PaymentVoucherService service)
        {
            _service = service;
        ...
        partial void OnSelectedItemChanged(PaymentVoucherLookupItem? oldValue, PaymentVoucherLookupItem? newValue)
        ...
        {
            var vm = App.ServiceProvider.GetRequiredService<PaymentVoucherEditViewModel>();
            vm.InitializeNew();
            var win = new PaymentVoucherEditWindow { DataContext = vm, Owner = Application.Current.MainWindow };
        ...
        {
            if (SelectedItem is null) return;
            if (MessageBox.Show("هل تريد حذف سند الصرف المحدد؟", "تأكيد", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
            {
        ...
        private static PaymentVoucherLookupItem Clone(PaymentVoucherLookupItem model) => new()
        {

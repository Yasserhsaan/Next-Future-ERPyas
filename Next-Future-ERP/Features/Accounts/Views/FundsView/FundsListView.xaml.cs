...
{
    /// <summary>
    /// Interaction logic for FundsListView.xaml
    /// </summary>
    public partial class FundsListView : Page
    {
        public FundsListView()
        {
            InitializeComponent();
            DataContext = App.ServiceProvider.GetRequiredService<FundsListViewModel>();
            
            // تأكد من أن CommandBar يحصل على DataContext
            Loaded += (s, e) =>
            {
                if (DataContext is FundsListViewModel vm)
                {

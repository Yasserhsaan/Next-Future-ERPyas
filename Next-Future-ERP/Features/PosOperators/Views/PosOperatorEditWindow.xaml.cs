...
{
    /// <summary>
    /// Interaction logic for PosOperatorEditWindow.xaml
    /// </summary>
    public partial class PosOperatorEditWindow : FluentWindow
    {
        private PosOperatorEditViewModel _viewModel;

        public PosOperatorEditWindow(PosOperatorEditViewModel viewModel)
        {
            InitializeComponent();
            ...
            Loaded += async (_, __) =>
            {
                try
                {
                    System.Diagnostics.Debug.WriteLine("PosOperatorEditWindow Loaded event fired");
                    if (DataContext is PosOperatorEditViewModel vm)
                    {
        ...
        private void AddEventHandlers()
        {
            try
            {
                ...
                var datePickers = FindVisualChildren<System.Windows.Controls.DatePicker>(this);
                foreach (var datePicker in datePickers)
                {
                    datePicker.SelectedDateChanged += (sender, args) => _viewModel?.RefreshCanSave();
                }
        ...
        public PosOperatorEditWindow()
        {
            ...
            Loaded += async (_, __) =>
            {
                try
                {
                    System.Diagnostics.Debug.WriteLine("PosOperatorEditWindow Loaded event fired (parameterless constructor)");
                    if (DataContext is PosOperatorEditViewModel vm)
                    {
                        System.Diagnostics.Debug.WriteLine("ViewModel found, calling InitializeAsync...");
                        await vm.InitializeAsync();
